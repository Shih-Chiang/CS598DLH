...
from gen_feature import gen_feature_vector
...
class BERT_CRF(nn.Module):
    ...
    def __init__(self):
        ...
        self.hidden2tag = nn.Linear(config.hidden_size, num_labels)
        self.crf = CRF(num_labels, batch_first=True)

    def forward(self, input_ids, tags, token_type_ids=None, input_mask=None, feature=None):
        emissions = self.tag_outputs(input_ids, token_type_ids, input_mask, feature=feature)
        loss = -1 * self.crf(emissions, tags, mask=input_mask.byte())

        return loss

    def tag_outputs(self, input_ids, token_type_ids=None, input_mask=None, feature=None):
    ...
    def predict(self, input_ids, token_type_ids=None, input_mask=None, feature=None):
        emissions = self.tag_outputs(input_ids, token_type_ids, input_mask, feature)
