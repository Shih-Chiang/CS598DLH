...
from gen_feature import gen_feature_vector2
...
class MyIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        word_array_set, token_array_set, feature_array_set, label_array_set, mask_array_set = [], [], [], [], []
        word_array, token_array, feature_array, label_array, mask_array = [], [], [], [], []
            ...
            for line in file_obj:
                if line == ' \n':
                    ...
                    if len(word_array) > maxlen:
                        ...
                        feature_array = feature_array[:maxlen]
                    ...
                    feature_array_set.append(feature_array)
                ...
                token = embedding_vector.key_to_index.get(w, 0)
                feature = gen_feature_vector2(w)
                ...
                if len(word_array_set) == batch_size:
                    yield np.array(padding2(word_array_set)), np.array(padding(token_array_set)), np.array(padding3(feature_array_set)) ,np.array(padding(label_array_set)), np.array(padding(mask_array_set))
                    word_array_set, token_array_set, feature_array_set, label_array_set, mask_array_set = [], [], [], [], []
...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm = nn.LSTM(325, config.lstm_dim, num_layers=1, bidirectional=True, batch_first=True)
    ...
    def forward(self, char_tensor, token_tensor, feature_tensor, tags, mask):
        emissions = self.tag_outputs(char_tensor, token_tensor, feature_tensor, mask)
        loss = -1 * self.crf(emissions, tags, mask=mask.byte())

        return loss

    def tag_outputs(self, char_tensor, token_tensor, feature_tensor, mask):

        char_embedding, (_, _) = self.char_lstm(char_tensor)
        word_embedding = self.embedding(token_tensor)

        combined_input = torch.cat((char_embedding, feature_tensor, word_embedding), dim=2)
    ...
    def predict(self, char_tensor, token_tensor, feature_tensor, mask):
        emissions = self.tag_outputs(char_tensor, token_tensor, feature_tensor, mask)
...
def evaluate(dataloader, model, id2label, all_ori_tokens):
    ...
    for b_i, (word, token, feature, label, mask) in enumerate(tqdm(dataloader, desc="Evaluating")):

        word = word[0].float().to(device)
        token = token[0].long().to(device)
        feature = feature[0].float().to(device)
        mask = mask[0].long().to(device)
        label = label[0].tolist()
        with torch.no_grad():
            logits = model.predict(word, token, feature, mask)
...
for epoch in range(config.epochs):
    ...
    for word, token, feature, label, mask in pbar:
        word = word.float().to(device)
        token = token.long().to(device)
        feature = feature.float().to(device)
        label = label.long().to(device)
        mask = mask.long().to(device)
        loss = model(word[0], token[0], feature[0], label[0], mask[0])
        loss.backward()
        optimizer.step()
        model.zero_grad()
